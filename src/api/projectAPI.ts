...
export const projectAPI = {
    ...
    */
    ...
    justificacion: string — justificación de los cambios (obligatoria, 10-500 caracteres).
    Autenticación mediante token JWT.
    Validación de permisos para modificar proyectos específicos.
    */
    editarProyecto: async (
        id: string,
        proyectoData: Omit<Proyecto, 'id_proyecto'>,
        justificacion: string
    ): Promise<Proyecto> => {
        const response = await API.put<Proyecto>(`/proyectos/${id}`, {
            ...proyectoData,
            justificacion
        });
        return response.data;
